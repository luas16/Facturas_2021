...
var vents = {
    ...
    calculate_invoice: function () {
        var subtotal = 0.00;
        var poriva = ($('input[name="aggiva"]').val())/100;
        ...
        this.items.subtotal = subtotal / (poriva+1);
        this.items.iva = this.items.subtotal * poriva;
        total = this.items.subtotal + this.items.iva;
        var descuento = (($('input[name="descuento"]').val()) / 1) * 1;
...
$(function () {
    ...
    $('#frmSale').on('submit', function (e) {
        ...
        vents.items.cli = $('input[name="empleado"]').val();
        ...
        submit_with_ajax(window.location.pathname, 'Notificación',
            '¿Estas seguro de realizar la siguiente acción?', parameters, function (response) {
                alert_action('Notificación', '¿Desea imprimir la boleta de venta?', function () {
                    window.open('/ctrl/fact/invoice/pdf/' + response.id + '/', '_blank'); //se abre una pagina en blanco con el pdf de la factura
                    location.href = '/ctrl/fact/list/';  //se redirige cuando se hace submit
                }, function () {
                    location.href = '/ctrl/fact/list/';
                });
            });
    });
